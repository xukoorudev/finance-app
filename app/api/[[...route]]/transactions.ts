...
const app = new Hono()
   ...
   .post(
      ...
      async (c) => {
         ...
         const transactionsToBulkDelete = db.$with("transactions_to_bulk_delete").as(
         ...
         const data = await db
            ...
            .delete(transactions)
            .where(
               inArray(transactions.id, sql`(select id from ${transactionsToBulkDelete})`)
            )
            .returning({
               id: transactions.id,
            })
   ...
   .delete(
      ...
      async (c) => {
         ...
         const transactionsToDelete = db.$with("transactions_to_dalete").as(
            db.select({ id: transactions.id})
            .from(transactions)
            .innerJoin(accounts, eq(transactions.accountId, accounts.id))
            .where(
